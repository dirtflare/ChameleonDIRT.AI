...
## 落とし穴：vault が Desktop 配下だと launchd から読めない（macOS TCC）

`~/Desktop` `~/Documents` `~/Downloads` は macOS の TCC（プライバシー保護）対象。
launchd が起動した子プロセス（`/bin/bash`）はここへのアクセスを拒否されるため、
**登録は成功するのに実行だけ静かに失敗する**。

### 見分け方

- エラーが `Operation not permitted`（EPERM）であって `Permission denied`（EACCES）ではない
  → Unix のパーミッション不足ではなく TCC ブロックのシグネチャ
- スクリプトは 755 で、対話セッションからは普通に読める（＝ファイル権限の問題ではない）
- `WorkingDirectory` すら効かず `getcwd` が失敗する（vault に到達する前、`~/Desktop` の時点で弾かれている）

`launchctl list` に載っていても「動く」の証明にはならない。必ず
`launchctl start <Label>` で即時実行し、ログに巡回レポートが出るところまで確認する。

### 対処：vault を Desktop の外へ移す

`~/CLAUDECODE/...` のようなホーム直下は TCC 対象外。ここへ移せば権限付与なしで通る。

`/bin/bash` にフルディスクアクセスを与える回避策もあるが**推奨しない**。以後この Mac で走る
あらゆるシェルスクリプトが全保護領域へアクセスできるようになり、ネットからコピペした
コマンドを実行する習慣があるなら被害範囲が Mac 全体に広がる。

なお、ラッパースクリプトだけ Desktop の外に置く回避策は効かない。拒否されているのは
「Desktop 配下のパスへのアクセス」そのもので、スクリプトの置き場所とは無関係。

### 移動時の注意

- 同一ボリュームなら `mv` はメタデータ操作のみで一瞬（コピーは発生しない）
- 相対シンボリックリンクは壊れない。絶対パス参照だけ置換すればよい
- **過去ログは書き換えない**（その時点の記録なので、書き換えると記録として嘘になる）
- `~/.claude.json` はパス keyed なので、新パスは別プロジェクト扱いになり権限 allowlist がリセットされる
  （実害は確認プロンプトが増えるだけ。稼働中セッションが終了時に上書きするので手で編集しない）
- 移動後は Claude Code を再起動する（プロセスは inode を掴んだままで `pwd` が古いパスを指す）
- Obsidian も `Open folder as vault` で新しい場所を開き直す
