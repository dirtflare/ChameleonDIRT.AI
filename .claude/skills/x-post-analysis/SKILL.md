...
その全ミラー(fxtwitter, nitter, syndication API, r.jina.ai 経由など)への
接続を拒否する(検証済み)。そのため X の投稿は URL からは絶対に読めない。
**ユーザーが貼り付けたテキストが唯一の入力**である。

大原則: **一発コピペで完結させる**。ユーザーに求めてよいのは
「投稿ページを丸ごとコピーして貼る」という 1 回の操作だけ。
項目を分けた追加提出を要求しない。貼られたものが乱雑でも、
そこから読み取れる限りを解析し、足りない部分は質問で止まらず
「不明点」として報告に含めて先へ進む。

## Step 0 — URL しか無い場合

フェッチは試みない(ブロック済みと分かっている)。1 回だけ、こう依頼する:

> この環境からは X を直接読めないため、投稿ページを開いて
> **全体を選択してコピーし、そのまま貼り付けてください**(整形不要)。
> 動画がある場合は、動画で話されている・映っている内容を
> 一言添えてもらえると解析の精度が上がります。

これ以上の要求はしない。貼り付けが来たら即 Step 1 へ。

## 貼り付けが不完全でも止まらない

- 投稿者名や日時が無い → 「不明」として進める
- 動画付きなのに動画の説明が無い → 本文とリンクから推測できる範囲で解析し、
  「動画の中身は未確認。取り込み判断は本文ベース」と明記する
- 本文がまったく無い(URL や数字だけ)場合のみ、再度コピペを依頼してよい

動画そのものは環境からは視聴できない。本文が短い動画ツイートは
価値の大半が動画側にあるため、動画内容の欠落は報告で必ず指摘する。
